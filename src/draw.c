...
/*   Updated: 2020/01/23 13:14:16 by jominodi    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
void		draw_column(t_env *env, t_ray *ray, int xy[3])
{
	...
	env->mrg[0] = (wall <= WIN_HEIGHT) ? (WIN_HEIGHT - wall) / 2 + env->jump[0] : env->jump[0];
	env->mrg[1] = (wall <= WIN_HEIGHT) ? (WIN_HEIGHT - wall) / 2 + env->jump[1] : env->jump[1];
	while (xy[1] - env->up < WIN_HEIGHT / 2)
	{
		if (xy[1] < env->mrg[0])
			color = 0x308FC9;
		else if (xy[1] > env->mrg[0] && xy[1] < env->mrg[0] + wall)
		{
			ft_memcpy(&color, &env->text[(int)ray->id].data[(((int)ray->mod) +
				64 * (64 * cmpt / wall)) * 4], sizeof(int));
			cmpt++;
		}
		else if (xy[1] > env->mrg[0] + wall)
			color = 0x95671F;
		// add_sprite(ray, )
		if (env->sick == 1)
			color *= 12 + 255;
