...
t_ray	*find_ver_wall(t_env *env, float ang)
{
		...
		(int)xy[1] / 64 >= 0 && (int)xy[1] / 64 < SIZE_MAP)
	{
		...
		{
			sprite->next = create_ray(sqrt(pow(env->cam.x - ((int)(xy[1] / 64) * 64 + 32) , 2) + pow(env->cam.y - ((int)(xy[0] / 64) * 64 + 32) , 2)), 0, 7);
			sprite = sprite->next;
...
t_ray	*find_hor_wall(t_env *env, float ang)
{
		...
		(int)xy[1] / 64 >= 0 && (int)xy[1] / 64 < SIZE_MAP)
	{
		...
		if (env->map[(int)xy[1] / 64][(int)xy[0] / 64].type == 'L')
		{
			sprite->next = create_ray(sqrt(pow(env->cam.x - ((int)xy[1] / 64 + 0.5) * 64, 2) + pow(env->cam.y - ((int)xy[0] / 64 + 0.5) * 64, 2)), 0, 7);
			sprite = sprite->next;
			dxdy[0] = (int)(env->cam.x + cos(ang * M_PI / 180) * sprite->dist) % 64;
			dxdy[1] = (int)(env->cam.y + sin(ang * M_PI / 180) * sprite->dist) % 64;
			sprite->mod = (env->cam.angle - ang == 0) ? 0 : 32+fabs((32 - dxdy[0]) + (32 - dxdy[1]));
